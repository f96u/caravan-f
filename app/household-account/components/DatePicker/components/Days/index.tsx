          ...
          {weeks(weekNum).map(({ day, isThisMonth }) => (
            <div key={day}>
              <button
                ...
                type="button"
                className={`m-px flex size-10 items-center justify-center rounded-full border border-transparent text-sm hover:border-blue-600 disabled:pointer-events-none disabled:text-gray-300 ${isCalendarDate(day) && isThisMonth ? 'pointer-events-none bg-blue-600 font-medium text-white': 'text-gray-800 hover:text-blue-600'}`}>
                {day}
              </button>
            </div>
          ))}
